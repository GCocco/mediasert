...
class NavMesh:
    _cddl = ctypes.cdll.LoadLibrary("./navmesh.so")
    ...
    class PathPointer:
        def __init__(self, path_list):
            self._head = ctypes.pointer(NavMesh.PathNode.from_address(path_list))
        ...
        def __del__(self):
            print(NavMesh)
            NavMesh._cddl.free_path(self._raw_point)
            return
    ...
    def __init__(self, filename):
        self._pointer = self._cddl.newNavmesh(ctypes.c_char_p(filename.encode("utf-8")))
        pass

    def _find(self, x, y):
        return Coordinate.from_address(self._cddl.find(self._pointer, ctypes.c_double(x), ctypes.c_double(y)))

    def find_path(self, start_x, start_y, end_x, end_y):
        return NavMesh._cddl.find_path(self._pointer,
                                    ctypes.c_double(start_x), ctypes.c_double(start_y),
                                    ctypes.c_double(end_x), ctypes.c_double(end_y))    
    def __del__(self):
        NavMesh._cddl.del_navmesh(self._pointer)
        return
    
    def navigate(self, pos_start, pos_end):
        ppointer = NavMesh.PathPointer(self.find_path(pos_start.getX(),
                                                      pos_start.getY(),
                                                      pos_end.getX(),
                                                      pos_end.getY()))
